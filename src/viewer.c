...
#include <X11/Xutil.h>
/* If you do not actually use XFixes, feel free to remove this line:
...
static void render_image(Display *dpy, Window win)
{
	...
	/*
	 * Change from 5-arg to 4-arg. 
	 * Old (some IM7 builds): MagickResizeImage(tmp_wand, w, h, LanczosFilter, 1.0);
	 */
	...
	XImage *ximg = XCreateImage(dpy, DefaultVisual(dpy, DefaultScreen(dpy)), 24, ZPixmap, 0,
	...
	XPutImage(dpy, win, gc, ximg, 0, 0, 0, 0, out_width, out_height);
...
int viewer_init(Display **dpy, Window *win, const char *filename, MsxivConfig *config)
{
	...
	*win = XCreateSimpleWindow(*dpy, RootWindow(*dpy, screen), 0, 0, win_w, win_h, 1,
...
void viewer_run(Display *dpy, Window win)
{
	...
	while (1) {
		...
		case KeyPress: {
			...
			if (g_command_mode) {
				if (ks == XK_Return) {
					...
					if (c >= 32 && c < 127 && g_command_len < (int)(sizeof(g_command_input) - 1)) {
			...
			/* Panning/zooming or config keybinds. */
			switch (ks) {
			case XK_q:
				return;
			case XK_w:
			...
			case XK_plus:
			case XK_equal:
				/* If '=' without shift => fit to window. */
				if (ks == XK_equal && !(ev.xkey.state & ShiftMask)) {
					fit_zoom(dpy, win);
					break;
				}
				/* else it's '+' with shift */
				g_zoom += ZOOM_STEP;
				...
				for (int i = 0; i < g_config->keybind_count; i++) {
					if (strcmp(g_config->keybinds[i].key, pressed_key) == 0) {
						...
						if (strncmp(g_config->keybinds[i].action, "save", 4) == 0) {
							...
							*p = '\0';
							/* 
							   system() returns an int, ignoring it triggers a warning.
							   If you want to handle return status, store it:
		...
		case MotionNotify:
			/* Example: if you wanted to implement click-and-drag panning,
			   you'd do it here using ev.xmotion.x, ev.xmotion.y, etc. */
			break;
		}
	}
}
